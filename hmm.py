...
def test():
  states = tf.constant([0,1])
  obervations = tf.constant([0,1,2])
  start = tf.constant([0.5,0.5])
  transition = tf.Variable(tf.truncated_normal([states.shape[0], states.shape[0]],
                            stddev=1.0 / math.sqrt(states.shape[0])))
  emission = tf.Variable(tf.truncated_normal([states.shape[0], obervations.shape[0]],
                            stddev=1.0 / math.sqrt(obervations.shape[0])))
  print(states.shape)
